...
export const PaginationSelectorStyles = (
  { pagination }: Theme,
  { isOpen, hasTotalResults }: { isOpen: boolean; hasTotalResults: boolean },
): SerializedStyles => {
  return css`
    position: relative;
    ...
    .dropdown-wrapper {
      position: absolute;
      padding: 0;
      max-width: ${hasTotalResults ? "4.5rem" : "50rem"};
      width: 100%;
      ${hasTotalResults ? "inset-inline-end: 0;" : "inset-inline-start: 0;"}
